/*
 * myLIMS API
 *
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace IO.Swagger.Models
{
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class Sample : IEquatable<Sample>
    { 
        /// <summary>
        /// Gets or Sets Id
        /// </summary>

        [DataMember(Name="Id")]
        public int? Id { get; set; }

        /// <summary>
        /// Gets or Sets Identification
        /// </summary>

        [DataMember(Name="Identification")]
        public string Identification { get; set; }

        /// <summary>
        /// Gets or Sets ControlNumber
        /// </summary>

        [DataMember(Name="ControlNumber")]
        public string ControlNumber { get; set; }

        /// <summary>
        /// Gets or Sets ControlIdentification
        /// </summary>

        [DataMember(Name="ControlIdentification")]
        public string ControlIdentification { get; set; }

        /// <summary>
        /// Gets or Sets Account
        /// </summary>

        [DataMember(Name="Account")]
        public BaseAccount Account { get; set; }

        /// <summary>
        /// Gets or Sets RelatedAccount
        /// </summary>

        [DataMember(Name="RelatedAccount")]
        public RelatedAccount RelatedAccount { get; set; }

        /// <summary>
        /// Gets or Sets SampleType
        /// </summary>

        [DataMember(Name="SampleType")]
        public SampleType SampleType { get; set; }

        /// <summary>
        /// Gets or Sets CollectionPoint
        /// </summary>

        [DataMember(Name="CollectionPoint")]
        public CollectionPoint CollectionPoint { get; set; }

        /// <summary>
        /// Gets or Sets Latitude
        /// </summary>

        [DataMember(Name="Latitude")]
        public float? Latitude { get; set; }

        /// <summary>
        /// Gets or Sets Longitude
        /// </summary>

        [DataMember(Name="Longitude")]
        public float? Longitude { get; set; }

        /// <summary>
        /// Gets or Sets ServiceCenter
        /// </summary>

        [DataMember(Name="ServiceCenter")]
        public ServiceCenter ServiceCenter { get; set; }

        /// <summary>
        /// Gets or Sets SampleConclusion
        /// </summary>

        [DataMember(Name="SampleConclusion")]
        public SampleConclusion SampleConclusion { get; set; }

        /// <summary>
        /// Gets or Sets SampleInfos
        /// </summary>

        [DataMember(Name="SampleInfos")]
        public List<SampleInfo> SampleInfos { get; set; }

        /// <summary>
        /// Gets or Sets SamplePriceItems
        /// </summary>

        [DataMember(Name="SamplePriceItems")]
        public List<SamplePriceItem> SamplePriceItems { get; set; }

        /// <summary>
        /// Gets or Sets TotalPrice
        /// </summary>

        [DataMember(Name="TotalPrice")]
        public float? TotalPrice { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class Sample {\n");
            sb.Append("  Id: ").Append(Id).Append("\n");
            sb.Append("  Identification: ").Append(Identification).Append("\n");
            sb.Append("  ControlNumber: ").Append(ControlNumber).Append("\n");
            sb.Append("  ControlIdentification: ").Append(ControlIdentification).Append("\n");
            sb.Append("  Account: ").Append(Account).Append("\n");
            sb.Append("  RelatedAccount: ").Append(RelatedAccount).Append("\n");
            sb.Append("  SampleType: ").Append(SampleType).Append("\n");
            sb.Append("  CollectionPoint: ").Append(CollectionPoint).Append("\n");
            sb.Append("  Latitude: ").Append(Latitude).Append("\n");
            sb.Append("  Longitude: ").Append(Longitude).Append("\n");
            sb.Append("  ServiceCenter: ").Append(ServiceCenter).Append("\n");
            sb.Append("  SampleConclusion: ").Append(SampleConclusion).Append("\n");
            sb.Append("  SampleInfos: ").Append(SampleInfos).Append("\n");
            sb.Append("  SamplePriceItems: ").Append(SamplePriceItems).Append("\n");
            sb.Append("  TotalPrice: ").Append(TotalPrice).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((Sample)obj);
        }

        /// <summary>
        /// Returns true if Sample instances are equal
        /// </summary>
        /// <param name="other">Instance of Sample to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(Sample other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    Id == other.Id ||
                    Id != null &&
                    Id.Equals(other.Id)
                ) && 
                (
                    Identification == other.Identification ||
                    Identification != null &&
                    Identification.Equals(other.Identification)
                ) && 
                (
                    ControlNumber == other.ControlNumber ||
                    ControlNumber != null &&
                    ControlNumber.Equals(other.ControlNumber)
                ) && 
                (
                    ControlIdentification == other.ControlIdentification ||
                    ControlIdentification != null &&
                    ControlIdentification.Equals(other.ControlIdentification)
                ) && 
                (
                    Account == other.Account ||
                    Account != null &&
                    Account.Equals(other.Account)
                ) && 
                (
                    RelatedAccount == other.RelatedAccount ||
                    RelatedAccount != null &&
                    RelatedAccount.Equals(other.RelatedAccount)
                ) && 
                (
                    SampleType == other.SampleType ||
                    SampleType != null &&
                    SampleType.Equals(other.SampleType)
                ) && 
                (
                    CollectionPoint == other.CollectionPoint ||
                    CollectionPoint != null &&
                    CollectionPoint.Equals(other.CollectionPoint)
                ) && 
                (
                    Latitude == other.Latitude ||
                    Latitude != null &&
                    Latitude.Equals(other.Latitude)
                ) && 
                (
                    Longitude == other.Longitude ||
                    Longitude != null &&
                    Longitude.Equals(other.Longitude)
                ) && 
                (
                    ServiceCenter == other.ServiceCenter ||
                    ServiceCenter != null &&
                    ServiceCenter.Equals(other.ServiceCenter)
                ) && 
                (
                    SampleConclusion == other.SampleConclusion ||
                    SampleConclusion != null &&
                    SampleConclusion.Equals(other.SampleConclusion)
                ) && 
                (
                    SampleInfos == other.SampleInfos ||
                    SampleInfos != null &&
                    SampleInfos.SequenceEqual(other.SampleInfos)
                ) && 
                (
                    SamplePriceItems == other.SamplePriceItems ||
                    SamplePriceItems != null &&
                    SamplePriceItems.SequenceEqual(other.SamplePriceItems)
                ) && 
                (
                    TotalPrice == other.TotalPrice ||
                    TotalPrice != null &&
                    TotalPrice.Equals(other.TotalPrice)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (Id != null)
                    hashCode = hashCode * 59 + Id.GetHashCode();
                    if (Identification != null)
                    hashCode = hashCode * 59 + Identification.GetHashCode();
                    if (ControlNumber != null)
                    hashCode = hashCode * 59 + ControlNumber.GetHashCode();
                    if (ControlIdentification != null)
                    hashCode = hashCode * 59 + ControlIdentification.GetHashCode();
                    if (Account != null)
                    hashCode = hashCode * 59 + Account.GetHashCode();
                    if (RelatedAccount != null)
                    hashCode = hashCode * 59 + RelatedAccount.GetHashCode();
                    if (SampleType != null)
                    hashCode = hashCode * 59 + SampleType.GetHashCode();
                    if (CollectionPoint != null)
                    hashCode = hashCode * 59 + CollectionPoint.GetHashCode();
                    if (Latitude != null)
                    hashCode = hashCode * 59 + Latitude.GetHashCode();
                    if (Longitude != null)
                    hashCode = hashCode * 59 + Longitude.GetHashCode();
                    if (ServiceCenter != null)
                    hashCode = hashCode * 59 + ServiceCenter.GetHashCode();
                    if (SampleConclusion != null)
                    hashCode = hashCode * 59 + SampleConclusion.GetHashCode();
                    if (SampleInfos != null)
                    hashCode = hashCode * 59 + SampleInfos.GetHashCode();
                    if (SamplePriceItems != null)
                    hashCode = hashCode * 59 + SamplePriceItems.GetHashCode();
                    if (TotalPrice != null)
                    hashCode = hashCode * 59 + TotalPrice.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(Sample left, Sample right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(Sample left, Sample right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
